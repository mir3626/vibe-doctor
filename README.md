...
- Softens synced agent memory and overview docs so Claude is described as the default nominal Orchestrator, not the only possible Orchestrator.
- Clarifies that direct Codex sessions can operate in Codex Orchestrator maintenance mode for upstream harness work.
- Removes the misleading Claude-specific escalation wording from Codex Generator memory.

---

## Previous highlights (v1.7.11)

### v1.7.11 (2026-05-08) - Sidecar dogfood hardening

- Fixes Windows/Codex sidecar execution by resolving the npm-installed Codex JS entrypoint instead of relying on `spawnSync("codex")`.
- Hardens sealed packet handling with `inputHash` verification, wrapper-owned coverage, semantic reviewer-output validation, and `--cwd` path consistency.
- Redacts sensitive tracked diffs, omits untracked contents by default, rejects artifact roots outside `--cwd`, and removes Claude adapter file-reading tools.

---

## Previous highlights (v1.7.10)
...
- Hardens `/vibe-review` harness-gap ledger parsing so escaped pipes and inline-code pipes do not shift six-column table cells.
- Keeps prior-review regression parsing aligned with the documented `## Findings (severity desc)` heading.
- Accepts legacy minimal `.vibe/agent/project-decisions.jsonl` bootstrap records without noisy schema warnings.
- Expands review/planning guidance for semantic agent-context, multimodal, and workflow-agent features to require task-quality evidence beyond shape/smoke checks.

---

## Previous highlights (v1.7.7)

### v1.7.7 (2026-05-06) - Interview consensus gate

- `/vibe-interview` now emits a `phase: "consensus"` gate before final `seedForProductMd` generation.
- Human approval, revision, deferred approval, and PO-proxy unconfirmed outcomes are recorded in the interview session and final Phase 3 seed.
- The final `product.md` seed includes a `Phase 3 Consensus Check` block so downstream planning can see whether the agent understanding was actually confirmed.

---

## Previous highlights (v1.7.6)
...
- Adds `vibe-context-audit` as the first P1-lite, report-only follow-up from the Matt Pocock skills review.
- Scans harness skills/runbooks for referenced paths, classifies dependency strength as `hard`, `soft`, or `unknown`, and reports `known`, `missing`, `ambiguous`, and `stale` counts.
- Intentionally does not gate preflight, sprint commit, Generator prompts, push, or tags.

---

## Previous highlights (v1.7.4)
...
- LTS baseline tag: `v1.7.3-lts` points to the same release commit as `v1.7.3`.
...
- LTS 선언은 immutable alias tag (`vMAJOR.MINOR.PATCH-lts`) 로 남긴다. moving `lts` tag 는 downstream exact pin 을 혼동시키므로 사용하지 않는다.
