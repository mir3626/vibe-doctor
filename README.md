...
| `vibe-stop-qa-gate.mjs` | 턴 종료 시 코드 diff 감지 → 새 코드 상태만 QA 실행, 성공 지문 재사용 |
...
현재 릴리스는 `harnessVersion: 1.7.28` 입니다. 릴리스를 자를 때는 `package.json`, `.vibe/config.json`, release note, tag를 같은 버전으로 맞춥니다.
