...
> ⚠️ **템플릿 주의**: `docs/context/product.md` 와 `docs/context/architecture.md` 는 의도적으로 플레이스홀더 상태입니다. **`git clone` 직후 `/vibe-init` 을 먼저 실행**해서 프로젝트 맥락을 채운 뒤 개발을 시작하세요. 플레이스홀더 상태로 Sprint 를 돌리면 Orchestrator/Planner 가 맥락 없이 작업하게 됩니다. v1.5.16부터 `/vibe-sync` 와 legacy sync bootstrap 도 init 전 실행을 중단합니다.
...
자세한 릴리스 내역은 `docs/release/v1.2.0.md` ~ `v1.5.16.md` 참조.
...
```
Sync plan: vibe-doctor v1.5.15 -> 1.5.16
...
cat .vibe/config.json | grep harnessVersion    # "1.5.16" 출력
```
