...
### Previous: v1.15.0 (2026-09-07) - Model-selected workflows and shared runtime correctness
...
> 필수 1회 실행. 생략하면 Stop 훅(`vibe-stop-qa-gate`) 이 `[vibe-harness-qa] skip: tsx not installed — run \`npm install\` first` 로 리포트하고, `npm run vibe:qa` / `vibe-validate-state` 등 tsx loader 를 쓰는 검증 커맨드가 모두 skip 된다. dev dependency (`tsx`, `typescript`, `@types/node`, `zod-to-json-schema`) 는 harness 스크립트가 런타임에 요구하므로 반드시 먼저 설치.
...
| `/vibe-goal-iterate` | 확정된 설계를 /goal + iterate 반복 구현 사이클로 자율 소진 (project-owned shared runbook) |
| `$vibe-pro-go` | Web Pro GitHub 설계/리뷰와 Codex 구현/보고를 최신 flow부터 자동 재개. `confirm-skip` 디렉티브가 켜져 있으면 발행 전 사용자 확인 생략 |
...
npm run vibe:init-ready         # init 산출물 존재 게이트 (pre-MVP Sprint 차단)
...
npm run vibe:roadmap-maintenance       # sprint-roadmap 활성 구간/아카이브 유지보수
npm run vibe:pro-go                    # 현재 repo/branch의 최신 Web Pro flow sync + 다음 행동
npm run vibe:pro-go -- doctor          # bridge/protocol 상태와 Web M0 수동 gate 점검
npm run vibe:pro-go -- confirm-skip on|off|status  # 발행 확인 skip 디렉티브 (config.local.json, opt-in)
...
| `vibe-init-ready.mjs` | `/vibe-init` 산출물 존재 게이트 — 미완료 시 Sprint prompt/구현 차단 |
| `vibe-roadmap-maintenance.mjs` | sprint-roadmap 활성 구간 유지보수 + 완료 iteration 아카이브 |
...
| `vibe-agent-session-start.mjs` | Provider-neutral SessionStart entrypoint; short-window duplicate hook delivery suppression + lifecycle provenance |
...
| `vibe-stop-qa-gate.mjs` | 턴 종료 시 하네스 소유 변경만 감지 → detached·hidden 하네스 QA 예약 후 즉시 반환, 잠금·결과 지문 재사용 |
...
| `vibe-pro-go.mjs` | GitHub append-only Web Pro ↔ Codex 설계/구현/리뷰 왕복 (`confirm-skip` 발행 확인 skip 디렉티브 포함) |
...
| `run-codex.sh` / `run-codex.cmd` | Codex CLI 래퍼 (UTF-8 + retry + `--health` + MD injection diagnostic + auto status-tick + session-start stdin 격리) |
...
├── README.md                      # 이 파일
├── bridge-runbook.md              # Web Pro GitHub 진입 런북 ($vibe-pro-go Web-first entry)
...
│   │   ├── src/pro-roundtrip/     # $vibe-pro-go flow/report/transport 런타임
│   │   ├── scripts/               # 하네스 실행 스크립트
│   │   ├── scripts/lib/           # render/open/interview/shard-audit helpers
│   │   ├── schemas/               # pro-roundtrip/sidecar JSON schemas
