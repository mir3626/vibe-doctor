...
| `vibe-stop-qa-gate.mjs` | 턴 종료 시 하네스 소유 변경만 감지 → detached 하네스 QA 예약 후 즉시 반환, 잠금·결과 지문 재사용 |
...
현재 릴리스는 `harnessVersion: 1.7.29` 입니다. 릴리스를 자를 때는 `package.json`, `.vibe/config.json`, release note, tag를 같은 버전으로 맞춥니다.
