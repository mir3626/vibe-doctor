...
| `$vibe-pro-go` | Web Pro GitHub 설계/리뷰와 Codex 구현/보고를 최신 flow부터 자동 재개 |
...
| `vibe-pro-go.mjs` | GitHub append-only Web Pro ↔ Codex 설계/구현/리뷰 왕복 |
...
현재 릴리스는 `harnessVersion: 1.7.30` 입니다. 릴리스를 자를 때는 `package.json`, `.vibe/config.json`, release note, tag를 같은 버전으로 맞춥니다.
