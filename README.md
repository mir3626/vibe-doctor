...
- `scripts/vibe-*.mjs`가 운영 참조(package/CLAUDE/AGENTS/GEMINI/settings/skill) 없이 생성됐거나 sync manifest에서 빠지면 review Blocker seed 후보로 드러납니다.
...
- Windows에서 `./scripts/run-codex.sh` provider wrapper를 실행할 때 bare `bash` 대신 Git Bash를 직접 탐색합니다.
...
- Windows에서 POSIX wrapper(`./scripts/run-codex.sh`)를 실행해야 할 때 harness CLI는 Git Bash를 직접 찾아 사용한다. 수동 실행은 `"C:\Program Files\Git\bin\bash.exe" scripts/run-codex.sh --health` 처럼 Git Bash 경로를 명시한다.
- WSL에서는 Windows npm shim(`/mnt/c/.../npm/codex`)을 재사용하지 않는다. WSL 안에 Linux용 `node`/`codex`를 별도로 설치하거나 Windows 쪽에서 `scripts\run-codex.cmd`를 사용한다.
...
cat .vibe/config.json | grep harnessVersion    # "1.6.8" 출력
```
...
- `vibe:run-agent` 는 Windows에서 `./scripts/run-codex.sh` 를 Git Bash로 직접 실행한다. Git Bash가 설치되어 있는지 먼저 확인.
- 수동 점검은 Git Bash 경로를 명시한다: `"C:\Program Files\Git\bin\bash.exe" scripts/run-codex.sh --health`.
- 네이티브 PowerShell/cmd health check는 `scripts\run-codex.cmd --health` 를 사용.
