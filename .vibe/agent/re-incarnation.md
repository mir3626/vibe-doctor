...
4. **`.vibe/agent/session-log.md`** — 직전 세션에서 Orchestrator가 append한 증분 저널.
   압축이 지운 mid-session 발견·결정·실패를 복원한다. handoff가 "스냅샷"이라면 이건 "저널".
5. **`.vibe/agent/handoff.md`** — narrative 상태 박제. Mission, Sprint 이력, last action,
   **Next action**. 여기 §5 "Next action"이 재부팅 후 첫 작업이다.
6. **관련 Sprint 프롬프트 / Generator final report** — handoff가 가리키는 파일만 선택적으로 읽는다.
