...
1. **`CLAUDE.md`** — 역할 제약, 트리거 매트릭스, 항상 지킬 것 (이건 auto-load 됨).
2. **auto-memory** — `MEMORY.md` 인덱스 → 관련 shard 읽기. 특히
   `feedback_language_tone.md`, `feedback_autonomous_execution.md`, 진행 중 프로젝트의
   `project_*.md`.
3. **`.vibe/agent/sprint-status.json`** — 현재 Sprint id, handoff 필드
   (`currentSprintId`, `lastActionSummary`, `openIssues`, `orchestratorContextBudget`,
   `preferencesActive`).
4. **`.vibe/agent/handoff.md`** — narrative 상태 박제. Mission, trigger matrix,
   P0 task list, last action, **Next action**. 여기 §7 "Next action"이 재부팅 후 첫
   작업이다.
5. **관련 Sprint 프롬프트 / Generator final report** — handoff가 가리키는 파일만.
