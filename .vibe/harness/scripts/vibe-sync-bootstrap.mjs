...
function expandHarnessEntries(upstreamRoot, entries) {
  const exact = [];
  const globs = [];
  for (const entry of entries) {
    if (isGlob(entry)) {
      ...
      exact.push(normalizeRelativePath(entry));
    }
  }
  ...
  const files = readdirSync(upstreamRoot, { recursive: true, withFileTypes: true })
    .filter((entry) => entry.isFile())
    .map((entry) => normalizeRelativePath(path.relative(upstreamRoot, path.join(entry.parentPath, entry.name))));

  return Array.from(
    new Set([
      ...exact,
      ...globs.flatMap((glob) => files.filter((candidate) => matchesGlob(glob, candidate))),
    ]),
...
function main() {
  ...
  try {
    ...
    for (const relativePath of expandHarnessEntries(upstreamRoot, manifest.files.harness ?? [])) {
