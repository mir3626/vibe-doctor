...
function runProjectReport() {
  const reportScript = resolve('.vibe/harness/scripts/vibe-project-report.mjs');
...
function tsxImportSpecifier(scriptDir) {
  ...
  const tsxLoader = candidates.find((candidate) => existsSync(candidate));
  return tsxLoader ? pathToFileURL(tsxLoader).href : 'tsx';
}
...
function runCli() {
  ...
  if (!sprintId || !status || !['passed', 'failed'].includes(status)) {
    fail(
      'Usage: node .vibe/harness/scripts/vibe-sprint-complete.mjs <sprintId> <passed|failed> [--summary "summary text"] [--scope <path1,path2,...>] [--no-auto-report]',
    );
  }
