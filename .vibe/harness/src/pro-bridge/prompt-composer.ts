...
function renderPrompt(input: ComposerInput, requestId: string): string {
  ...
  return [
    ...
    'When Vibe Pro Bridge mailbox tools are attached to this conversation:',
    'The task is incomplete until the Bridge returns status=result-ready.',
    'After generating every required result file, call publish_review_package.',
    'Do not finish by only printing Markdown in chat.',
    'If publish_review_package returns chunked-upload-required, upload every requested file or chunk and call finalize_result.',
    ...
    'Calling begin_result alone is not completion. Do not cancel and recreate a request to avoid a publication conflict.',
    'When mailbox tools are not attached (the Phase 1 manual wire), outputting the one complete vibe-bundle block from section G is the completion path.',
    'For that path, Phase 1 is manual: output the final response as one complete vibe-bundle block. The user will copy it into the CLI importer. A truncated copy without the VIBE:END sentinel is rejected.',
    '',
