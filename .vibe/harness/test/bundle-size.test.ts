...
async function compileBundleSize(root: string): Promise<string> {
  ...
  await execFile(
    ...
    [
      ...
      '--rootDir',
      sourceRoot,
      path.join(sourceRoot, '.vibe', 'harness', 'src', 'commands', 'bundle-size.ts'),
      path.join(sourceRoot, '.vibe', 'harness', 'src', 'lib', 'cli.ts'),
      path.join(sourceRoot, '.vibe', 'harness', 'src', 'lib', 'config.ts'),
      path.join(sourceRoot, '.vibe', 'harness', 'src', 'lib', 'fs.ts'),
      path.join(sourceRoot, '.vibe', 'harness', 'src', 'lib', 'paths.ts'),
    ],
    {
      cwd: sourceRoot,
      env: process.env,
  ...
  return path.join(outDir, '.vibe', 'harness', 'src', 'commands', 'bundle-size.js');
}
