...
4. Add one concise entry immediately below the first `## Entries` heading in `.vibe/agent/session-log.md`, using a full ISO timestamp with timezone and a useful tag such as `[decision]`, `[harness-review]`, or `[checkpoint]`. This newest-first placement matches the Sprint writer. Do not append below an archive or rewrite/reorder older records; review also reads legacy preamble/tail entries by timestamp.
