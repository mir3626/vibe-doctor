...
2. **AI Agent CLI/플러그인 확인** — `claude`, `codex` 플러그인 등 존재 여부를 확인합니다.
   - Codex는 CLI 대신 **Claude Code 플러그인** (`openai/codex-plugin-cc`)으로 사용합니다.

3. **ouroboros 설치 확인** — `python -m ouroboros --version`으로 ouroboros-ai 패키지 설치 여부를 확인합니다.
   - 미설치인 경우: `pip install --user ouroboros-ai` 설치를 안내하고, 사용자 승인 후 설치합니다.
   - 설치 후 `.mcp.json`에 ouroboros MCP 서버가 등록되어 있는지 확인합니다.
...
2. Generator (코드 구현 — 체크리스트 기반 구현) [기본: codex (플러그인)]
...
|----------|-----------|-----------|
| codex | Claude Code에서 `/plugin marketplace add openai/codex-plugin-cc` 실행 후 `/reload-plugins` | 플러그인 설치 시 자동 연결. OpenAI API 키가 필요한 경우 플러그인 안내에 따라 설정 |
...
- 인터뷰 도구가 응답하지 않거나 에러가 발생하면, 사용자에게 상황을 알리고 수동 Q&A로 전환합니다.
