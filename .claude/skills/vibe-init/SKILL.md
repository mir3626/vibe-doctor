...
2. **AI Agent CLI 확인** — `claude`, `codex`, `gemini` 등 CLI 존재 여부를 확인합니다.
   - 결과를 사용자에게 보여줍니다. 예:
     ```
     환경 점검 결과:
       node (v22.x)  ✓
...
## Phase 2 — Provider 역할 배정 및 인증

사용자에게 3가지 역할에 대해 **각각** 어떤 AI를 사용할지 물어봅니다.
...
```
AI Agent 역할을 설정합니다. 각 역할에 어떤 AI를 사용할지 선택해주세요.
기본값은 괄호 안에 표시됩니다. 그대로 쓰려면 엔터만 누르면 됩니다.

1. Coder (코드 작성) [기본: codex]
   선택지: codex / gemini / claude / 기타

2. Challenger (검증/대안 제시) [기본: gemini]
   선택지: gemini / codex / claude / 기타

3. Reviewer (리뷰) [기본: claude]
   선택지: claude / codex / gemini / 기타
...
```
커스텀 AI agent 설정:
  - provider 이름은 뭔가요? (예: deepseek, grok, cursor, aider)
...
| gemini | `npm install -g @anthropic-ai/gemini` 또는 시스템 패키지 관리자 | `gemini` 실행 후 브라우저에서 Google 계정 로그인 |
| claude | Claude Code 사용 중이므로 이미 인증됨 | - |
...
선택한 provider 구성을 반영합니다. 커스텀 provider는 `providers` 맵에 추가합니다.

예시 (coder를 deepseek로 선택한 경우):
```json
{
...
coder로 선택된 provider에 맞게 파일 내용을 수정합니다.
- 기본 codex가 아닌 경우, 파일 상단의 provider 이름과 역할 설명을 변경합니다.
- 예: coder가 deepseek인 경우:
  ```markdown
  # DeepSeek project memory

  너의 기본 역할은 **코드 작성기(coder)** 다.
  ...
  ```

#### `GEMINI.md`

challenger로 선택된 provider에 맞게 파일 내용을 수정합니다.
- 기본 gemini가 아닌 경우:
  - 파일명은 그대로 유지하되, 내용에서 provider 이름을 변경합니다.
  - 또는 challenger가 gemini가 아니면 해당 파일에 실제 challenger provider 이름을 반영합니다.
- 예: challenger가 grok인 경우:
  ```markdown
  # Grok project memory
...
## Main orchestrator
- Claude
- 계획 수립 / 승인 게이트 유지 / QA orchestration / context 관리 / 보고서 작성

## Default coder
- 기본값: {선택된 coder}
- 실제 코드 작성 / 테스트 가능성 보강 / 최소 범위 변경
...
초기 세팅이 완료되었습니다!
  ...
  AI Agent 설정:
    Orchestrator : Claude (기본)
    Coder        : {선택된 coder}  {✓ 인증됨 / ⚠ 미연결}
    Challenger   : {선택된 challenger}  {✓ 인증됨 / ⚠ 미연결}
    Reviewer     : {선택된 reviewer}

  작성/수정된 파일:
    - .vibe/config.local.json     (provider 설정)
    - docs/context/product.md     (프로젝트 목표)
    - docs/context/architecture.md (기술 스택)
    - docs/context/conventions.md  (코드 규칙)
    - AGENTS.md                    (coder 규칙)
    - GEMINI.md                    (challenger 규칙)
    - docs/orchestration/roles.md  (역할 배정)

이제 목표를 말씀해주시면 바로 작업을 시작할 수 있습니다.
...
- AGENTS.md와 GEMINI.md는 파일명은 유지하되, 내용만 선택된 provider에 맞게 수정합니다.
