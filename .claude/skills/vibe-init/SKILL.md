...
> **CRITICAL — Phase 3는 스킵 불가**: 사용자가 "자율 진행 / 위임 / 알아서 해" 등을
> 지시해도 **인터뷰 flow 자체는 건너뛰지 않습니다**. 인터뷰의 가치는 Q&A 수집이 아니라
> **숨겨진 가정을 강제로 드러내는 구조적 탐색**에 있습니다. 답변자가 사용자이든 PO 대행
> 모드의 Orchestrator이든 flow는 완주합니다. Ouroboros 설치 실패(3회 재시도 모두 실패)
> 시에만 fallback(§Phase 1 말미) 이 허용됩니다.
...
#### PO 대행 모드 (사용자가 "자율 진행 / 위임" 을 지시한 경우)

Orchestrator가 **사용자를 대신해서** ouroboros의 질문에 답변합니다. 원칙:

- 질문 한 건이 올 때마다 extended thinking으로 **판단 후 답변**. "대충 넘기기" 금지.
- 답변은 `product.md` 의 성공 기준·플랫폼·가정과 **일관성**을 유지. 인터뷰 후반의 답변이 전반 답변과 모순되지 않도록 self-check.
- "확실하지 않음" 이 판단인 경우는 억지 답변 금지. ouroboros에 "미정" 으로 전달.
- ouroboros에 보낼 답변과 별개로, **사용자에게 "이 질문에 나는 이렇게 답할게"를 한 문장으로 공유** (채팅 UI 1줄). 사용자가 오버라이드할 기회 제공. 오버라이드 없으면 Orchestrator 답변으로 진행.

> **rationale 기록은 Phase 종료 시 1회만**: 매 답변마다 기록 X. 모호성 점수 ≤ 0.2 도달 후
> 인터뷰가 종료되면 session-log에 다음 형식으로 append (한 번만):
>
> ```
> - YYYY-MM-DDTHH:mm [decision][phase3-po-proxy] Phase 3 완주.
>   핵심 답변 N개 rationale: (1) <Q 요약> → <A 요약> ∵ <근거>; (2) ...; (3) ...
>   최종 Ambiguity: 0.XX. 주요 미정 항목: <list 또는 "none">.
> ```
>
> 로그 폭증 방지 + 향후 재구성 가능한 최소 기록. 매 답변 rationale이 필요하면 향후 dogfood
> 반복에서 수치 조정.
...
- **PO 대행 모드 문서화**: 인터뷰 종료 후 `product.md` 말미에 `## Phase 3 답변 기록 (PO 대행)` 섹션을 추가하여 Q/A 전체 덤프를 남깁니다 (사용자가 PO 대행을 사용한 경우에만). 향후 Planner/Evaluator가 의심스러운 판단 근거를 역추적할 수 있도록 합니다.
