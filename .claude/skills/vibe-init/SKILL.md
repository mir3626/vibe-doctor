...
- `mode=human`: Phase 2 Fast-path 은 사용자가 "기본" 등으로 답했을 때만 활성. Phase 3 는 사용자 상호작용 기본.
- `mode=agent`: Phase 2 Fast-path + Phase 3 PO-proxy 가 자동 활성. 사용자 추가 질문 최소화.

> 분기 로직의 실제 구현은 이 flag 가 사용되는 각 Phase/Step 의 기본 서술에 점진적으로
> 반영됩니다 (iteration-3 N3 에서 5 분기점 공식 지정). 현 단계에서는 값만 기록하고
> 각 Phase 의 기존 fast-path / PO-proxy 조건을 그대로 유지합니다.
