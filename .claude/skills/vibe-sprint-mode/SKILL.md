...
- `git push --force`, `git reset --hard`, `git clean`, `git branch -D`, `git restore`, destructive checkout/rebase
- 모든 `rm` / `rm -rf`
- `npm publish`
...
- **on**: 지정한 tier 의 preset 을 `.claude/settings.local.json` 의 `permissions.allow` / `permissions.deny` 에 dedupe merge. Legacy `Bash(cmd:*)` preset entries are cleaned up when enabling.
- **off**: 양 tier preset 의 allow/deny rule 을 모두 제거. 사용자 커스텀 rule 은 유지.
- **status**: tier 별 active allow/deny rule 수 + 활성 tier 표시.
...
- extended tier 도 `Bash` 전체 접근은 제공하지 않음. 다만 interruption 을 줄이기 위해 명시 command family 는 wildcard 로 묶는다.
