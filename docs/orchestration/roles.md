...
> **호출 방법**:
> - **Planner / Evaluator** (Claude 계열) → `Agent` 도구 사용 (model: "opus")
> - **Generator** (Codex) → `Bash("codex exec ...")` 로 CLI 직접 호출. Agent 도구는 Claude만 지원하므로 Generator에 사용 금지.
> - **기타 비-Claude 계열** provider → Bash 도구로 CLI/API 명령 실행
>
...
// .vibe/config.json (커밋 대상 — 프로젝트 공통 기본값)
{
  ...
  "providers": {
    ...
    "codex": { "command": "codex", "args": ["exec", "--json", "{prompt}"], "note": "Codex CLI로 직접 호출" }
  }
}
```
