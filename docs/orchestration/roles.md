...
> - **Claude 계열** provider → Agent 도구 사용 (model 파라미터 지정)
> - **Codex** → **Codex 플러그인** 사용. `codex:codex-rescue` 에이전트 (Agent 도구, `subagent_type: "codex:codex-rescue"`) 또는 `codex:rescue` 스킬 (Skill 도구)로 호출한다. Bash 도구로 CLI를 직접 호출하지 않는다.
> - **기타 비-Claude 계열** provider (gemini 등) → Bash 도구로 CLI/API 명령 실행 (`.vibe/config.json` → `providers` 섹션의 command/args 참조)
>
...
// .vibe/config.json (커밋 대상 — 프로젝트 공통 기본값)
{
  ...
  "providers": {
    ...
    "codex": { "plugin": "codex:codex-rescue", "note": "Codex 플러그인 에이전트로 호출" }
  }
}
```
