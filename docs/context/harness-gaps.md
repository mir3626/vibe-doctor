...
| gap-direct-vibe-init-shell-entrypoint | Users can run `npm run vibe:init` from bash/cmd/PowerShell, which creates files but cannot complete agent-required product context, roadmap, handoff, and session-log work. | `.vibe/harness/src/commands/init.ts` direct-shell guard + `.claude/skills/vibe-init/SKILL.md` guarded command + `.vibe/harness/test/init-guard.test.ts` (v1.5.15). Agent skills use `--from-agent-skill`; direct shells exit with guidance. | covered | covered | ??|
