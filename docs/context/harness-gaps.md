...
| gap-sync-ref-stale-pin | Downstream `.vibe/config.json.upstream.ref` 가 설치 당시 tag(예: `v1.4.3`)로 남아 있으면 statusline/version-check 이 최신 tag 를 알려줘도 `/vibe-sync` 가 기존 ref 를 우선하여 같은 버전을 다시 clone. | `src/commands/sync.ts` cached latestVersion ref resolution + `test/sync.test.ts` `resolveUpstreamRef` regression (v1.5.10). `--ref` 와 non-version branch refs remain explicit overrides. | covered | covered | — |
