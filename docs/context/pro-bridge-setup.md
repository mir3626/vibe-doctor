...
1. **리뷰는 Pro 챗에서** — 패킷 붙여넣기(manual wire)와 함께 outbox의 `range.diff`를 첨부한다. Pro는 도구가 없으므로 committed base..head 근거는 `range.diff`, local-only dirty 근거는 인라인 또는 첨부 `patch.diff`가 담당한다. `range-stat.txt`에서 예산 때문에 빠진 파일과 secret/binary 제외 로스터를 확인한다.
2. **제출은 두 경로 중 하나**:
...
CLI-origin 요청은 committed `base..head` 범위를 `git diff --no-renames`로 수집해 manual outbox와 mailbox request 디렉터리에 항상 `<requestDir>/range.diff`와 `<requestDir>/range-stat.txt`를 생성한다. 기본 `proBridge.rangeDiffBudget.maxBytes`는 2 MiB다. 예산을 넘으면 파일 중간을 자르지 않고 파일 단위로 제외하며, 포함/제외 로스터와 diffstat를 `range-stat.txt`에 기록한다. Pro 챗에는 패킷을 붙여넣고 `range.diff`를 첨부한다. 작은 range diff는 기존 64 KiB 인라인 예산 안에서 프롬프트에도 포함된다.

로컬 patch가 있으면 상한 내 patch는 manual과 mailbox 양쪽의 review prompt에 fenced diff로 포함된다. manual outbox에는 크기와 무관하게 `<requestDir>/patch.diff`도 생성된다. 인라인 상한을 넘으면 CLI가 해당 파일을 리뷰 대화에 직접 첨부하라고 안내한다. mailbox wire는 상한 초과 patch를 별도로 가져오는 도구가 아직 없으므로, 이 경우 manual artifact 첨부 경로를 사용한다.
