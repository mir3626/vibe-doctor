...
- 네이티브 인터뷰 진행 (사용자와 대화 또는 PO 대행)
- Sprint 로드맵 분할 (`docs/plans/sprint-roadmap.md`, current iteration only)
...
|---|---|---|---|---|
| **Orchestrator** | 기본 nominal mode: Claude Opus (메인 대화). Codex 직접 대화 세션은 maintenance mode에서 가능 | 상주 | 세션 전체 누적 | Phase 생명주기, 인터뷰 진행, Sprint 로드맵, 상태 유지, sub-agent 소환, 사용자 소통 |
...
| **Phase 3** 네이티브 인터뷰 | ○ **PO 대행 포함 완주** (스킵 금지) | — | — | — |
...
파일 경로는 `docs/plans/sprint-roadmap.md`. 이 active 파일에는 현재 iteration만 유지하고,
완료되었거나 비활성화된 iteration 섹션은 `docs/plans/archive/roadmaps/<iter-id>.md` 로
보존한다. 매 Sprint 시작 시 Planner에게 해당 slot만 전달.
...
2. 완료 체크리스트 — 기계 검증 항목과 inspection/demo 항목을 분리. 기계 검증은 `tsc`, test, grep 등으로 확인하고, 제품 정체성·사용감·시각/상호작용 품질처럼 자동화하기 어려운 항목은 Evaluator 또는 사용자 inspection 대상으로 명시
   - frontend/game/visual/canvas/WebGL/Three.js/editor/dashboard 등 경험형 제품이면 screenshot, Stagehand UI-test output, browser-smoke output, 또는 playthrough note 중 하나 이상의 evidence item을 반드시 포함한다.
...
| "자율 진행" → 네이티브 인터뷰 스킵 | 숨겨진 가정 발견 장치 우회. 재현성 ↓ | PO 대행으로 인터뷰 완주 |
...
- **`.vibe/harness/scripts/vibe-preflight.mjs`** — 파일 존재 체크 (향후 code 확장 예정)
