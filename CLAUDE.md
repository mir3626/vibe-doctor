...
기획 품질이 최종 산출물 품질을 결정한다. 단계별 최소 추론 강도:

| 단계 | 최소 추론 강도 | 강제 메커니즘 |
|------|---------------|-------------|
| Phase 0 (요구사항 발견) | **maximum** | Ouroboros 인터뷰 + extended thinking |
| Sprint 분할 / Planner | **maximum** | Planner sub-agent는 fresh context에서 최대 추론 |
| Sprint 프롬프트 작성 | **high** | Orchestrator 자체 검증 (프롬프트 QA 체크리스트) |
| 코드 생성 | Generator 기본 | `run-codex.sh` (자동 규칙 주입) |
| 검증 | **high** | 기계적 검증 + Evaluator (트리거 시) |
...
**원칙**: 스크립트가 FAIL을 반환하면 다음 단계로 진행하지 않는다.
...
0. **Phase 0 — 요구사항 발견 (새 프로젝트 시 필수)**: 사용자 목표가 3문장 이하 또는
   `docs/context/product.md`가 없으면 Ouroboros PM 인터뷰(`ouroboros_pm_interview`)를
   실행하여 타겟 유저·핵심 기능·우선순위·제약사항을 도출하고 `docs/context/product.md`에
   기록한다. `vibe-preflight.mjs`가 product.md 부재를 FAIL로 보고하므로 **건너뛸 수 없다**.
1. 사용자 목표 → Orchestrator가 Sprint 단위로 분할 → 사용자 승인. Planner Must 트리거 시에만 Planner 소환하며, Planner는 **기술 사양(타입 정의, API 시그니처, 파일 구조) + Sprint 프롬프트 초안**까지 출력한다. Orchestrator는 Planner 출력물에 최소한의 편집(메타데이터, 포맷)만 추가하여 Generator에 전달한다.
2. Sprint 시작 전 `node scripts/vibe-preflight.mjs` 실행 — git/deps/provider/상태/handoff staleness/product.md 기계적 체크. 새 프로젝트는 `--bootstrap` 플래그 사용.
3. Sprint 내: 트리거 매트릭스에 따라 필요한 역할만 소환. 최소 구성은 **Generator(코드) + Orchestrator self-QA**. Generator는 체크리스트를 만족하는 한 자유롭게 구현, Evaluator는 체크리스트 외 기준으로 불합격 판정하지 않는다.
4. **Sprint 완료 시** `node scripts/vibe-sprint-complete.mjs <sprintId> <passed|failed>` 실행 — sprint-status.json 갱신 + handoff.md 갱신 + session-log.md 타임스탬프 자동 append. **수동 갱신 대신 이 스크립트가 상태 파일을 관리한다.**
5. sub-agent는 Sprint 내에서만 존재하고 Sprint 간 context를 공유하지 않는다. Sprint 간 상태는 `.vibe/agent/sprint-status.json` + `handoff.md` + `session-log.md`로만 전달하고, **context 압축 직후**에는 작업 전에 먼저 이 세 파일 + 관련 memory shard를 읽어 상태를 복원한다.

## Sprint 프롬프트 작성 원칙

Planner가 Sprint 프롬프트를 작성할 때, Orchestrator가 보충할 때 모두 적용:
...
## 관련 스킬
`/vibe-init`, `/goal-to-plan`, `/self-qa`, `/write-report`, `/maintain-context`, `/vibe-sync`.
