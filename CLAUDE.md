...
### Dashboard attention hooks

- `hooks.Notification` runs `node scripts/vibe-attention-notify.mjs`, which records Claude Code permission/auth prompts in `.vibe/agent/attention.jsonl` and the UTC daily JSONL shard.
- Harness scripts append dashboard history through `node scripts/vibe-daily-log.mjs <type> --payload '<json>'` as a fire-and-forget side channel. The Orchestrator must not read `.vibe/agent/daily/*`, `attention.jsonl`, or `dashboard.pid`.
- For LLM-level attention events, the Orchestrator may run `node scripts/vibe-attention.mjs --severity urgent --title "..." --detail "..."`.
