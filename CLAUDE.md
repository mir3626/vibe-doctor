...
2. Sprint 시작 전 `node scripts/vibe-preflight.mjs` 실행 — git/deps/provider/상태/handoff staleness 기계적 체크.
...
4. sub-agent는 Sprint 내에서만 존재하고 Sprint 간 context를 공유하지 않는다. Sprint 간 상태는 `.vibe/agent/sprint-status.json` + `handoff.md` + `session-log.md`로만 전달하고, **context 압축 직후**에는 작업 전에 먼저 이 세 파일 + 관련 memory shard를 읽어 상태를 복원한다.
