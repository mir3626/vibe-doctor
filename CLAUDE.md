...
| 턴 종료 시 (Stop) | `node .vibe/harness/scripts/vibe-stop-qa-gate.mjs` | sync manifest 기준 하네스 소유 변경만 감지 → detached worker에 `vibe:typecheck` + `vibe:self-test` 예약 후 즉시 반환. 제품 QA는 실행하지 않으며 동일 상태·동시 Stop은 잠금/결과 지문으로 중복 제거 |
