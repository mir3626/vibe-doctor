...
describe('Codex agent TOML', () => {
  ...
  it('sync manifest ships Codex agent role files and this regression test', async () => {
    ...
    assert.equal(manifest.files.harness.includes('test/codex-agents.test.ts'), true);
    assert.equal(manifest.files.harness.includes('migrations/1.6.2.mjs'), true);
    assert.equal(manifest.migrations['1.6.2'], 'migrations/1.6.2.mjs');
  });
...
`,
      ...
      await execFileAsync(process.execPath, [path.join(process.cwd(), 'migrations', '1.6.2.mjs'), root]);
